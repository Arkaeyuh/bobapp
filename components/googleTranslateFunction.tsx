...
export async function translatePage(languageTo:string, languageFrom:string="en", landingPageToEN:boolean=false, haveButton=true) {
  ...
  document.documentElement.lang = languageTo;
  ...
  if (languageTo === languageFrom) return;
  ...
  if(!haveButton) stringOfElementsToSelect = 'p,span,h1,h2,h3,h4,h5,h6,label,option'
  ...
  try {
    ...
    if (!response.ok) throw new Error("Failed to translate page"); 
